...
sys.path.append(
    os.path.join(os.path.dirname(__file__), "..", "src", "modules", "cognitive", "symbolic")
)
...
from symbolic_ai_kamil import (
    ...
    SI_PREFIXES,
    # Integration
    integrate_with_discrepancy_gate,
...
class TestPydanticDataModels:
    ...
    def test_expression_validation(self):
        ...
        expr = Expr(
            expression_id="test_expr_1", terms=terms, infix_form="v = 9.8 * t", parse_confidence=0.9
        )
    ...
    def test_sym_clause_validation(self):
        ...
        with pytest.raises(ValueError, match="Logical form cannot be empty"):
            SymClause(clause_id="invalid", clause_type="fact", predicate="test", logical_form="")
...
class TestSymbolicAIPipeline:
    ...
    def test_dimensional_analysis_accuracy(self, symbolic_ai_engine):
        ...
        if total_predictions > 0:
            ...
            print(
                f"Dimensional analysis accuracy: {accuracy:.2f} ({correct_predictions}/{total_predictions})"
            )
...
def unit_strategy(draw):
    ...
    si_powers = tuple(Decimal(draw(st.integers(min_value=-3, max_value=3))) for _ in range(7))
    scale_factor = Decimal(
        str(draw(st.floats(min_value=1e-10, max_value=1e10, allow_nan=False, allow_infinity=False)))
    )
...
class TestPerformanceAndStress:
    ...
    def test_latency_slo_compliance(self):
        ...
        for input_text in test_inputs:
            ...
            assert (
                latency_ms <= 1000
            ), f"Latency SLO violation: {latency_ms:.2f}ms for '{input_text}'"
...
class TestNFCSIntegration:
    ...
    def test_neural_field_state_processing(self):
        ...
        # Add some structure
        x, y = np.meshgrid(
            np.linspace(0, 2 * np.pi, field_shape[0]), np.linspace(0, 2 * np.pi, field_shape[1])
        )
