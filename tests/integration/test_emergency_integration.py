...
class EmergencyIntegrationTest:
    ...
    async def _trigger_emergency(self) -> bool:
        ...
        try:
            self.logger.critical(f"🚨 ЗАПУСК АВАРИЙНОГО РЕЖИМА НА ШАГЕ {self.emergency_trigger_step}")
    ...
    async def _run_emergency_and_recovery_phase(self) -> bool:
        ...
        try:
            ...
            while step < self.total_steps:
                ...
                emergency_protocols = self.orchestrator.components.get('emergency_protocols')
                emergency_status = emergency_protocols.get_current_status() if emergency_protocols else {}
                ...
                if (step >= self.recovery_check_step and 
                    not recovery_detected and 
                    step_data['systemic_risk'] < 0.3 and
                    step_data['coherence_global'] > 0.6):
    ...
    async def _generate_test_report(self):
        ...
        try:
            ...
            # Общая оценка
            test_success = (
                self.test_results['emergency_triggered'] and
                self.test_results['emergency_detected'] and
                self.test_results['recovery_achieved'] and
                self.test_results['system_stable'] and
                not self.test_results['error_log']
            )
    ...
    def generate_visualization(self, save_path: Optional[str] = None):
        ...
        try:
            ...
            axes[0, 0].axvline(x=self.recovery_check_step, color='green', linestyle='--', 
                              label=f'Recovery Check (step {self.recovery_check_step})')
            axes[0, 0].axhline(y=0.3, color='red', linestyle=':', alpha=0.7, label='Critical Threshold')
            ...
            axes[0, 1].axhline(y=0.6, color='green', linestyle=':', alpha=0.7, label='Recovery Threshold')
            ...
            axes[1, 0].plot(steps, hallucination_number, 'purple', linewidth=2, label='Hallucination Number')
            ...
            axes[1, 0].axhline(y=0.8, color='red', linestyle=':', alpha=0.7, label='Emergency Threshold')
            ...
            axes[1, 1].plot(steps, [1-c for c in coherence_global], 'b-', label='1 - Global Coherence', alpha=0.8)
            axes[1, 1].axvline(x=self.emergency_trigger_step, color='orange', linestyle='--', 
                              label='Emergency Trigger')
            axes[1, 1].axvline(x=self.recovery_check_step, color='green', linestyle='--', 
