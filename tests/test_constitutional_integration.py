...
class TestConstitutionalRealTimeMonitor:
    ...
    async def test_algorithm_1_emergency_mode(self, monitor):
        ...
        assert all(
            isinstance(signal, (int, float, np.ndarray))
...
class TestEarlyWarningSystem:
    ...
    async def test_warning_suppression(self, warning_system):
        ...
        await warning_system._evaluate_prediction_warnings(
            warning_system.current_predictions.get(PredictionHorizon.SHORT_TERM), 2.0, 1.0, 0.5, 0.7
        )
    ...
    async def test_integrated_monitoring_flow(self, integrated_system):
        ...
        for step in range(20):
            ...
            if step == 5:  # Early stage
                assert monitor.current_status in [
                    ConstitutionalStatus.NORMAL,
                    ConstitutionalStatus.MONITORING,
                ]
        ...
        assert (
            monitor.current_status == ConstitutionalStatus.EMERGENCY
            or len(monitor.active_alerts) > 0
        )
...
class TestStressAndResilience:
    ...
    async def test_error_recovery(self):
        ...
        async def failing_metrics_callback():
            ...
            return {"hallucination_number": 0.5, "coherence_measure": 0.8, "integrity_score": 0.9}
        ...
        try:
            ...
            for i, metrics in enumerate(test_metrics):
                ...
                if i % 5 == 0:
                    ...
                    print(
                        f"Step {i}: Status={status['status']}, Warnings={len(warnings)}, "
