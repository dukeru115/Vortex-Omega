...
# System Information
system_info = Info("vortex_omega_system", "System information", registry=registry)
...
esc_buffer_size = Gauge(
    "esc_buffer_size", "Current ESC buffer size", ["buffer_type"], registry=registry
)

esc_memory_usage = Gauge("esc_memory_usage_mb", "ESC memory usage in MB", registry=registry)

esc_oscillator_count = Gauge(
    "esc_oscillator_count", "Number of active ESC oscillators", registry=registry
)

esc_echo_activations = Counter(
    "esc_echo_activations_total", "Total ESC echo activations", ["scale"], registry=registry
)

# Kuramoto metrics
kuramoto_order_parameter = Gauge(
    "kuramoto_order_parameter", "Kuramoto model order parameter (coherence)", registry=registry
)

kuramoto_frequency_variance = Gauge(
    "kuramoto_frequency_variance", "Variance of Kuramoto oscillator frequencies", registry=registry
)

kuramoto_sync_events = Counter(
    "kuramoto_sync_events_total", "Total Kuramoto synchronization events", registry=registry
)

# Discrepancy Gate metrics
discrepancy_detected = Counter(
    "discrepancy_detected_total", "Total discrepancies detected", ["severity"], registry=registry
)
...
rate_limit_exceeded = Counter(
    "rate_limit_exceeded_total", "Total rate limit violations", ["user_type"], registry=registry
)
...
db_query_duration = Histogram(
    "db_query_duration_seconds", "Database query duration", ["query_type"], registry=registry
)
...
cache_evictions = Counter(
    "cache_evictions_total", "Total cache evictions", ["cache_type", "reason"], registry=registry
)

# System metrics
cpu_usage = Gauge("system_cpu_usage_percent", "System CPU usage percentage", registry=registry)
...
disk_usage = Gauge(
    "system_disk_usage_percent", "System disk usage percentage", ["mount_point"], registry=registry
)
...
class MetricsCollector:
    ...
    def update_esc_metrics(
        self, buffer_sizes: Dict[str, int], memory_mb: float, oscillator_count: int
    ):
        """Update ESC metrics"""
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                duration = time.time() - start
                symbolic_processing_duration.labels(operation=f"{operation}_error").observe(
                    duration
                )
                raise
