...
class SystemEvent(BaseModel):
    ...
    affected_components: List[str] = Field(
        default_factory=list, description="System components affected by this event"
    )
...
class TelemetryUpdate(BaseModel):
    ...
    coherence_global: Optional[float] = Field(
        default=None, ge=0.0, le=1.0, description="Global neural field coherence"
    )
    coherence_modular: Optional[float] = Field(
        default=None, ge=0.0, le=1.0, description="Modular coherence measure"
    )
    hallucination_risk: Optional[float] = Field(
        default=None, ge=0.0, le=1.0, description="Current hallucination risk level"
    )
    defect_density: Optional[float] = Field(
        default=None, ge=0.0, description="Topological defect density"
    )
    active_modules: int = Field(..., ge=0, description="Number of active modules")
    memory_usage_mb: Optional[float] = Field(
        default=None, ge=0.0, description="Current memory usage"
    )
    cpu_usage_percent: Optional[float] = Field(
        default=None, ge=0.0, le=100.0, description="CPU utilization percentage"
    )
...
class EmergencyAlert(BaseModel):
    ...
    trigger_values: Optional[Dict[str, float]] = Field(
        default=None, description="Metric values that triggered the alert"
    )
    thresholds: Optional[Dict[str, float]] = Field(
        default=None, description="Threshold values that were exceeded"
    )
    auto_actions: List[str] = Field(
        default_factory=list, description="Automated actions taken in response"
    )
...
class WebSocketConnectionInfo(BaseModel):
    ...
    subscription_filters: List[EventType] = Field(
        default_factory=list, description="Event types this connection is subscribed to"
    )
    is_authenticated: bool = Field(default=False, description="Authentication status")
    metadata: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional connection metadata"
    )
...
class WebSocketSubscriptionRequest(BaseModel):
    ...
    filters: Optional[Dict[str, Any]] = Field(default=None, description="Event filtering criteria")
...
class WebSocketSubscriptionResponse(BaseModel):
    ...
    active_subscriptions: List[EventType] = Field(
        default_factory=list, description="Currently active subscriptions"
    )
...
class HeartbeatMessage(BaseModel):
    ...
    server_time: datetime = Field(
        default_factory=datetime.utcnow, description="Server current time"
    )
...
class ErrorMessage(BaseModel):
    ...
    retry_after: Optional[int] = Field(
        default=None, ge=0, description="Suggested retry delay in seconds"
    )
