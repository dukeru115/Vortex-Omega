...
class SemanticField(BaseModel):
    ...
    field_topology: Optional[Dict[str, float]] = Field(
        default=None, description="Topological characteristics"
    )
    attention_weights: Optional[List[float]] = Field(
        default=None, description="Multi-scale attention weights"
    )
...
class TokenAnalysis(BaseModel):
    ...
    semantic_embedding: Optional[List[float]] = Field(
        default=None, description="High-dimensional semantic embedding"
    )
    attention_score: float = Field(..., ge=0.0, le=1.0, description="Attention weight (0-1)")
    constitutional_compliance: float = Field(
        ..., ge=0.0, le=1.0, description="Constitutional safety score (0-1)"
    )
    semantic_anchors: Optional[List[str]] = Field(
        default=None, description="Identified semantic anchor points"
    )
    risk_factors: Optional[Dict[str, float]] = Field(
        default=None, description="Risk factor analysis"
    )
...
class ConstitutionalFilter(BaseModel):
    ...
    overall_compliance: float = Field(
        ..., ge=0.0, le=1.0, description="Overall constitutional compliance (0-1)"
    )
    policy_violations: List[str] = Field(
        default_factory=list, description="List of violated policies"
    )
    filtered_tokens: List[str] = Field(
        default_factory=list, description="Tokens filtered by constitutional constraints"
    )
    safety_score: float = Field(..., ge=0.0, le=1.0, description="Safety assessment score (0-1)")
    enforcement_actions: Optional[List[str]] = Field(
        default=None, description="Applied enforcement actions"
    )
...
class ESCProcessRequest(BaseModel):
    ...
    processing_mode: ProcessingMode = Field(
        default=ProcessingMode.STANDARD, description="ESC processing mode"
    )
    ...
    enable_constitutional_filtering: bool = Field(
        default=True, description="Enable constitutional safety filtering"
    )
    return_embeddings: bool = Field(
        default=False, description="Include semantic embeddings in response"
    )
    attention_layers: Optional[List[int]] = Field(
        default=None, description="Specific attention layers to analyze"
    )
    custom_parameters: Optional[Dict[str, Any]] = Field(
        default=None, description="Custom processing parameters"
    )
...
class ESCProcessResponse(BaseModel):
    ...
    token_analyses: List[TokenAnalysis] = Field(..., description="Per-token analysis results")
    ...
    sequence_coherence: float = Field(
        ..., ge=0.0, le=1.0, description="Overall sequence coherence score (0-1)"
    )

    # Optional detailed results
    attention_maps: Optional[List[List[float]]] = Field(
        default=None, description="Multi-scale attention maps"
    )
    semantic_graph: Optional[Dict[str, Any]] = Field(
        default=None, description="Semantic relationship graph"
    )
    performance_metrics: Optional[Dict[str, float]] = Field(
        default=None, description="Processing performance metrics"
    )
    warnings: Optional[List[str]] = Field(
        default=None, description="Processing warnings or notices"
    )
...
class ESCConfiguration(BaseModel):
    ...
    semantic_dimensions: int = Field(
        default=512, gt=0, le=2048, description="Semantic embedding dimensions"
    )
    attention_heads: int = Field(default=8, gt=0, le=32, description="Number of attention heads")
    constitutional_threshold: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Constitutional filtering threshold"
    )
    enable_adaptive_vocabulary: bool = Field(
        default=True, description="Enable adaptive vocabulary learning"
    )
    max_sequence_length: int = Field(
        default=512, gt=0, le=2048, description="Maximum sequence length"
    )
    cache_embeddings: bool = Field(default=True, description="Cache semantic embeddings")
...
class ESCSystemStatus(BaseModel):
    ...
    constitutional_violations: int = Field(
        ..., ge=0, description="Total constitutional violations detected"
    )
