...
class WebSocketConnection:
    ...
    async def send_error(self, error_code: str, message: str, details: Optional[str] = None):
        ...
        error_msg = ErrorMessage(error_code=error_code, error_message=message, details=details)
...
class ConnectionManager:
    ...
    async def broadcast_event(
        ...
        message = WebSocketMessage(
            event_type=event_type, source=source, data=data, sequence_id=self.total_messages_sent
        )
    ...
    async def send_telemetry_update(self, telemetry: TelemetryUpdate):
        ...
        await self.broadcast_event(
            EventType.TELEMETRY_UPDATE, "nfcs_orchestrator", telemetry.model_dump()
        )

    async def send_emergency_alert(self, alert: EmergencyAlert):
        """Send emergency alert to all connections"""
        await self.broadcast_event(
            EventType.EMERGENCY_ALERT, "emergency_controller", alert.model_dump()
        )
    ...
    async def send_system_event(
        ...
        await self.broadcast_event(EventType.SYSTEM_STATUS, "system", event_data)
            ...
            try:
                ...
                if self.connections:
                    ...
                    heartbeat = HeartbeatMessage(
                        uptime_seconds=uptime, active_connections=len(self.connections)
                    )

                    await self.broadcast_event(
                        EventType.HEARTBEAT, "websocket_manager", heartbeat.model_dump()
                    )
