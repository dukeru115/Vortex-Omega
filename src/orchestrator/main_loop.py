...
class OrchestratorStatistics:
    ...
    def update_cycle_metrics(self, cycle_metrics: CycleMetrics):
        ...
        if cycle_metrics.success:
            ...
            self.avg_cycle_time_ms = self.avg_cycle_time_ms * 0.9 + cycle_time * 0.1
            ...
            if cycle_time > 0:
                ...
                self.avg_frequency_hz = self.avg_frequency_hz * 0.9 + current_freq * 0.1
        else:
...
class NFCSMainOrchestrator:
    ...
    async def _initialize_components(self) -> bool:
        ...
        try:
            ...
            self.components["bus"] = await initialize_global_bus(
                max_buffer_size=10000, enable_telemetry=True
            )
            ...
            self.components["metrics_calculator"] = MetricsCalculator(
                self.system_config.cost_functional
            )
            ...
            self.components["emergency_protocols"] = create_default_emergency_protocols(
                enable_auto_detection=True, enable_auto_recovery=True
            )
            ...
            if self._main_task:
                ...
                try:
                    await asyncio.wait_for(
                        self._main_task, timeout=self.config.shutdown_timeout_seconds
                    )
    ...
    async def _main_coordination_loop(self):
        ...
        while self._running:
            ...
            try:
                ...
                if (
                    self.config.enable_detailed_telemetry
                    and cycle_number % self.config.telemetry_interval_cycles == 0
                ):
    ...
    async def _collect_system_state(self) -> bool:
        ...
        try:
            ...
            if "metrics_calculator" in self.components:
                self.current_system_state.risk_metrics = self.components[
                    "metrics_calculator"
                ].calculate_all_metrics(self.current_system_state)
        ...
        try:
            ...
            if "bus" in self.components:
                ...
                performance_payload = TelemetryPayload(
                    ...
                    module_state={
                        "cycle_number": (
                            self.current_cycle.cycle_number if self.current_cycle else 0
                        ),
        ...
        try:
            ...
            if "cgl_solver" in self.components and self.current_system_state:
                ...
                self.current_system_state.neural_field = cgl_solver.step(
                    self.current_system_state.neural_field, control_signals.u_field
                )
    ...
    async def _handle_emergency_conditions(self):
        ...
        try:
            ...
            emergency_detected = (
                risk_metrics.hallucination_number > ha_threshold
                or risk_metrics.rho_def_mean > defect_threshold
                or risk_metrics.coherence_global < 0.2
                or risk_metrics.coherence_modular < 0.2
            )
            ...
            if emergency_detected and self.current_system_state.system_mode == "NORMAL":
                ...
                not emergency_detected and self.current_system_state.system_mode == "EMERGENCY_MODE"
            ):
        ...
        try:
            ...
            detailed_payload = TelemetryPayload(
                ...
                module_state={
                    ...
                    "system_mode": self.current_system_state.system_mode,
                    "field_mean_amplitude": float(
                        np.mean(np.abs(self.current_system_state.neural_field))
                    ),
                    "field_max_amplitude": float(
                        np.max(np.abs(self.current_system_state.neural_field))
                    ),
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            status = {
                ...
                "components": {
                    name: type(component).__name__ for name, component in self.components.items()
                },
                "configuration": {
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"NFCSMainOrchestrator(state={self.state.value}, "
...
async def create_nfcs_orchestrator(
    config: Optional[OrchestratorConfig] = None, system_config: Optional[SystemConfig] = None
    ...
    async def demo_main_orchestrator():
        ...
        try:
            ...
            for i in range(20):
                ...
                print(
                    f"Цикл {status['statistics']['total_cycles']}: "
