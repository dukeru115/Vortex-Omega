...
class EventHandler:
    ...
    def handle_event(self, event: BusEvent) -> bool:
        ...
        try:
            ...
            logging.getLogger(__name__).error(f"Error in event handler {self.handler_id}: {e}")
            return False
...
class ResonanceBus:
    ...
    def _setup_internal_telemetry(self):
        ...
        self.subscribe(
            "internal_telemetry", telemetry_handler, topic_filter={TopicType.TELEMETRY_EVENT}
        )
    ...
    def publish(
        ...
        try:
            ...
            with self._buffer_lock:
                ...
                # Creation события
                event = BusEvent(
                    topic=topic, payload=payload, priority=priority, ttl_seconds=ttl_seconds
                )
                ...
                with self._stats_lock:
                    ...
                    self._stats["peak_buffer_size"] = max(
                        self._stats["peak_buffer_size"], len(self._event_buffer)
                    )
    ...
    async def _process_events_async(self):
        ...
        while self._is_running:
            try:
                ...
                await loop.run_in_executor(self._executor, self._process_event_sync, event)
        ...
        while self._is_running:
            try:
                ...
                with self._buffer_lock:
                    ...
                    active_events = [
                        event for event in self._event_buffer if not event.is_expired()
                    ]
    ...
    async def _publish_telemetry(self, event_name: str, data: Dict[str, Any]):
        ...
        self.publish(TopicType.TELEMETRY_EVENT, telemetry_payload, priority=EventPriority.LOW)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._handlers_lock:
            stats["active_handlers"] = len([h for h in self._handlers.values() if h.is_active])
        ...
        with self._handlers_lock:
            return {
                handler_id: {
                    ...
                    "priorities": (
                        list(handler.priority_filter) if handler.priority_filter else "ALL"
                    ),
                }
                for handler_id, handler in self._handlers.items()
            }
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"ResonanceBus(running={self._is_running}, "
...
def publish_eta_signal(
    eta_value: float, resonance_frequencies: List[float], source_module: str = "esc"
) -> bool:
...
def publish_risk_metrics(
    ...
    source_module: str = "risk_monitor",
) -> bool:
...
def publish_emergency(
    ...
    severity_level: int,
    trigger_reason: str,
    source_module: str = "emergency_controller",
) -> bool:
    ...
    return get_global_bus().publish(
        TopicType.ORCHESTRATION_EMERGENCY, payload, priority=EventPriority.EMERGENCY
    )
