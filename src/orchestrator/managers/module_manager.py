...
class ModuleInterface(ABC):
    ...
    async def handle_emergency(
        self, emergency_type: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
...
class ModuleManager:
    ...
    async def unregister_module(self, name: str) -> bool:
        ...
        try:
            ...
            if self.event_system:
                await self.event_system.emit_event(
                    "module.unregistered",
                    {"module_name": name, "timestamp": datetime.now(timezone.utc).isoformat()},
                )
            ...
            try:
                ...
                if success:
                    ...
                    if self.performance_monitor:
                        await self.performance_monitor.record_module_event(
                            name, "start", time.time() - start_time
                        )
        ...
        try:
            ...
            with self._module_lock:
                ...
                if module_info.dependents:
                    active_dependents = [
                        dep
                        for dep in module_info.dependents
                        if self._modules.get(dep, {}).status == ModuleStatus.RUNNING
                    ]
                    if active_dependents:
                        self.logger.warning(
                            f"Stopping module {name} with active dependents: {active_dependents}"
                        )
            ...
            try:
                ...
                self.logger.info(
                    f"Module {name} {'stopped successfully' if success else 'failed to stop'}"
                )
                return success
        ...
        try:
            with self._module_lock:
                ...
                return {
                    ...
                    "registration_time": (
                        module_info.registration_time.isoformat()
                        if module_info.registration_time
                        else None
                    ),
                    "start_time": (
                        module_info.start_time.isoformat() if module_info.start_time else None
                    ),
                    "last_heartbeat": (
                        module_info.last_heartbeat.isoformat()
                    ...
                    "success_rate": (
                        module_info.successful_operations / max(1, module_info.total_operations)
                    )
                    * 100,
    ...
    def _topological_sort(self) -> List[str]:
        ...
        modules_by_priority = sorted(
            self._modules.keys(), key=lambda name: self._modules[name].priority.value
        )
    ...
    async def _check_dependencies(self, module_name: str) -> bool:
        ...
        for dep_name in module_info.dependencies:
            ...
            if dep_status != ModuleStatus.RUNNING:
                self.logger.error(
                    f"Dependency {dep_name} not running for module {module_name} (status: {dep_status})"
                )
                return False
            ...
            for dep_name in module_info.dependencies:
                if dep_name in self._modules:
                    ...
                    if dep_status == ModuleStatus.UNREGISTERED:
                        self.logger.error(
                            f"Dependency {dep_name} not initialized for module {name}"
                        )
                        return False
    ...
    async def _perform_health_checks(self):
        ...
        with self._module_lock:
            active_modules = [
                name for name, info in self._modules.items() if info.status == ModuleStatus.RUNNING
            ]
        ...
        try:
            ...
            with self._module_lock:
                ...
                if module_info.health_score < 0.5:
                    self.logger.warning(
                        f"Module {module_name} health score low: {module_info.health_score}"
                    )

                    # Emit health warning event
                    if self.event_system:
                        await self.event_system.emit_event(
                            "module.health_warning",
                            {
    ...
    async def handle_module_emergency(
        self, module_name: str, emergency_type: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def __repr__(self) -> str:
        ...
        with self._module_lock:
            running_count = sum(
                1 for info in self._modules.values() if info.status == ModuleStatus.RUNNING
            )
