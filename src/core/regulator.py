...
class Regulator:
    ...
    def compute_feedback_control(
        self, state: SystemState, target_coherence: float = 0.7
    ) -> ControlSignals:
        """
        ...
        # Clip to constraints
        control_field = np.clip(np.abs(control_field), 0, self.max_control_amplitude) * np.exp(
            1j * np.angle(control_field)
        )
        ...
        return ControlSignals(
            u_field=control_field, u_modules=control_modules, timestamp=state.simulation_time
        )
