...
def _compute_kuramoto_derivatives_jit(phases: np.ndarray, frequencies: np.ndarray,
                                     coupling_matrix: np.ndarray, control_inputs: np.ndarray,
                                     global_coupling: float) -> np.ndarray:
    """
...
@njit(parallel=True, fastmath=True, cache=True)
def _runge_kutta4_step_jit(phases: np.ndarray, frequencies: np.ndarray,
                          coupling_matrix: np.ndarray, control_inputs: np.ndarray,
                          global_coupling: float, dt: float) -> np.ndarray:
    """
    ...
    # Compute k1
    k1 = _compute_kuramoto_derivatives_jit(phases, frequencies, coupling_matrix, 
                                         control_inputs, global_coupling)
    
    # Compute k2
    phases_k2 = phases + 0.5 * dt * k1
    k2 = _compute_kuramoto_derivatives_jit(phases_k2, frequencies, coupling_matrix,
                                         control_inputs, global_coupling)
    
    # Compute k3
    phases_k3 = phases + 0.5 * dt * k2
    k3 = _compute_kuramoto_derivatives_jit(phases_k3, frequencies, coupling_matrix,
                                         control_inputs, global_coupling)
    
    # Compute k4
    phases_k4 = phases + dt * k3
    k4 = _compute_kuramoto_derivatives_jit(phases_k4, frequencies, coupling_matrix,
                                         control_inputs, global_coupling)
...
@njit(parallel=True, fastmath=True, cache=True)
def _compute_local_order_parameters_jit(phases: np.ndarray, 
...
class OptimizedKuramotoSolver:
    ...
    def _setup_oscillators(self):
        ...
        if hasattr(self.config, 'coupling_matrix') and self.config.coupling_matrix is not None:
            ...
            self.coupling_matrix = np.ones((self.n_oscillators, self.n_oscillators), dtype=np.float64)
    ...
    def _warm_up_jit_functions(self):
        ...
        # Warm up functions
        _compute_kuramoto_derivatives_jit(test_phases, test_frequencies, test_coupling, test_control, 1.0)
        _runge_kutta4_step_jit(test_phases, test_frequencies, test_coupling, test_control, 1.0, 0.01)
    ...
    def step(self, phases: np.ndarray, control_inputs: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        new_phases = _runge_kutta4_step_jit(
            self._phases, self.frequencies, self.coupling_matrix,
            self._control_inputs, self.coupling_strength, self.dt
        )
    ...
    def evolve(self, initial_phases: np.ndarray, num_steps: int,
              control_inputs: Optional[np.ndarray] = None,
              store_trajectory: bool = False,
              convergence_check: bool = True) -> Tuple[np.ndarray, Dict[str, Any]]:
        """
        ...
        self.logger.info(f"Starting Kuramoto evolution: {num_steps} steps, {self.n_oscillators} oscillators")
        ...
        for step in range(num_steps):
            ...
            if (step + 1) % (num_steps // 10) == 0 or step == 0:
                ...
                self.logger.info(f"Evolution progress: {progress:.1f}%, R={R:.3f} (ETA: {eta:.1f}s)")
        ...
        self.logger.info(f"Evolution completed: {metrics['steps_per_second']:.1f} steps/sec, final R={metrics['final_order_parameter']:.3f}")
        
        return current_phases, metrics
    
    def _check_convergence(self, recent_order_params: List[float], 
    ...
    def compute_synchronization_metrics(self, phases: np.ndarray) -> Dict[str, float]:
        ...
        is_locked, locking_strength = _detect_phase_locking_jit(phases, self.phase_locking_tolerance)
        ...
        metrics = {
            ...
            'fraction_synchronized': np.sum(local_order > self.synchronization_threshold) / self.n_oscillators
        }
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        theoretical_speedup = f"~{50 if self.n_oscillators < 100 else 100 if self.n_oscillators < 1000 else 200}x"
        ...
        return {
            ...
            'oscillators_per_second': self.n_oscillators / avg_step_time if avg_step_time > 0 else 0,
