...
```
Динамика нейронных полей (CGL) ←→ Synchronization когнитивных модулей (Kuramoto)
           ↓                                         ↓
...
├── state.py                   # 🎯 Control состоянием системы и отслеживание
├── regulator.py              # ⚖️ Регуляторные механизмы и control
...
- `ψ(x,t)`: Amplitude комплексного нейронного поля
- `c₁, c₂, c₃`: Параметры системы, управляющие динамикой
- Описывает формирование паттернов, stability и топологические дефекты
...
**Параметры**:
- `θᵢ`: Phase осциллятора i
- `ωᵢ`: Собственная frequency осциллятора i
- `K`: Сила связи
