...
class EnhancedMetricsCalculator(MetricsCalculator):
    ...
    def calculate_constitutional_score(
        ...
        if hallucination_number <= self.constitutional_limits.max_hallucination_number:
            h_score = 1.0 - (
                hallucination_number / self.constitutional_limits.max_hallucination_number
            )
        else:
            h_score = 0.0
        ...
        if abs(energy_growth_rate) <= self.constitutional_limits.max_energy_growth_rate:
            e_score = (
                1.0 - abs(energy_growth_rate) / self.constitutional_limits.max_energy_growth_rate
            )
        else:
            e_score = 0.0
    ...
    def calculate_stability_index(
        self, field_history: List[np.ndarray], phase_history: List[np.ndarray]
    ) -> float:
        """
    ...
    def assess_emergency_conditions(
        self, current_metrics: RiskMetrics, advanced_metrics: AdvancedMetrics
    ) -> Tuple[bool, List[str]]:
        """
        ...
        if current_metrics.systemic_risk > 0.95:
            emergency_conditions.append(
                f"Critical systemic risk: {current_metrics.systemic_risk:.3f}"
            )

        # Extremely high hallucination number
        if (
            current_metrics.hallucination_number
            > 2 * self.constitutional_limits.max_hallucination_number
        ):
            emergency_conditions.append(
                f"Extreme hallucination: {current_metrics.hallucination_number:.3f}"
            )

        # Coherence collapse
        if current_metrics.coherence_global < 0.1 and current_metrics.coherence_modular < 0.1:
        ...
        # Energy runaway
        if (
            abs(advanced_metrics.energy_dissipation_rate)
            > 5 * self.constitutional_limits.max_energy_growth_rate
        ):
            emergency_conditions.append(
                f"Energy runaway: {advanced_metrics.energy_dissipation_rate:.3f}"
            )
        ...
        if self.enable_advanced_analysis:
            ...
            if len(self.energy_history) >= 2:
                dt = max(
                    ...
                    - getattr(
                        list(self.metric_history)[-1], "timestamp", state.simulation_time - 0.01
                    ),
                )
            ...
            advanced_metrics.defect_hierarchy = self.calculate_multi_scale_defects(
                state.neural_field
            )
            ...
            # Stability analysis
            recent_fields = [
                getattr(h, "neural_field", state.neural_field)
                for h in list(self.metric_history)[-10:]
                if hasattr(h, "neural_field")
            ]
            recent_phases = [
                getattr(h, "module_phases", state.module_phases)
                for h in list(self.metric_history)[-10:]
                if hasattr(h, "module_phases")
            ]
            ...
            advanced_metrics.stability_index = self.calculate_stability_index(
                recent_fields, recent_phases
            )
            ...
            safety_margins = [
                ...
                - (
                    base_metrics.hallucination_number
                    / self.constitutional_limits.max_hallucination_number
                ),
                1.0 - (base_metrics.rho_def_mean / self.constitutional_limits.max_defect_density),
                base_metrics.coherence_global / self.constitutional_limits.min_global_coherence
                - 1.0,
                base_metrics.coherence_modular / self.constitutional_limits.min_modular_coherence
                - 1.0,
            ]
            ...
            advanced_metrics.instability_warning = (
                advanced_metrics.stability_index < 0.3
                or anomaly_score > 0.8
                or abs(energy_growth_rate) > 2 * self.constitutional_limits.max_energy_growth_rate
            )

            # Emergency assessment
            emergency_detected, emergency_reasons = self.assess_emergency_conditions(
    ...
    def generate_safety_report(self) -> Dict[str, Any]:
        ...
        recent_coherence = [
            m.coherence_global for m in recent_metrics if hasattr(m, "coherence_global")
        ]
        recent_hallucinations = [
            m.hallucination_number for m in recent_metrics if hasattr(m, "hallucination_number")
        ]
        ...
        return {
            ...
            "risk_analysis": {
                ...
                "risk_violations": len(
                    [r for r in recent_risks if r > self.constitutional_limits.max_systemic_risk]
                ),
            },
            "coherence_analysis": {
                ...
                "coherence_violations": len(
                    [
                        c
                        for c in recent_coherence
                        if c < self.constitutional_limits.min_global_coherence
                    ]
                ),
            },
            "hallucination_analysis": {
                "mean_hallucinations": (
                    np.mean(recent_hallucinations) if recent_hallucinations else 0.0
                ),
                "max_hallucinations": (
                    np.max(recent_hallucinations) if recent_hallucinations else 0.0
                ),
                "hallucination_trend": self.predict_trend("hallucination_number"),
                "hallucination_violations": len(
                    [
                        h
                        for h in recent_hallucinations
                        if h > self.constitutional_limits.max_hallucination_number
                    ]
                ),
            },
    ...
    def _calculate_overall_compliance(self) -> float:
        ...
        for metrics in recent_metrics:
            if hasattr(metrics, "hallucination_number") and hasattr(metrics, "systemic_risk"):
                ...
                h_compliance = (
                    1.0
                    if metrics.hallucination_number
                    <= self.constitutional_limits.max_hallucination_number
                    else 0.0
                )
                r_compliance = (
                    1.0
                    if metrics.systemic_risk <= self.constitutional_limits.max_systemic_risk
                    else 0.0
                )
                c_compliance = (
                    1.0
                    if metrics.coherence_global >= self.constitutional_limits.min_global_coherence
                    else 0.0
                )
    ...
    def _generate_safety_recommendations(self) -> List[str]:
        ...
        if len(self.constitutional_violations) > 10:
            recommendations.append(
                "High violation rate detected - review constitutional parameters"
            )

        # Check emergency activations
        if self.computation_stats["emergency_activations"] > 3:
            recommendations.append(
                "Multiple emergency activations - system may need fundamental review"
            )
        ...
        if risk_trend > 0.1:
            recommendations.append(
                f"Rising risk trend detected ({risk_trend:.3f}) - consider preventive measures"
            )

        coherence_trend = self.predict_trend("coherence_global")
        if coherence_trend < -0.05:
            recommendations.append(
                f"Declining coherence trend ({coherence_trend:.3f}) - review coupling parameters"
            )

        # Check computation performance
        if self.computation_stats["average_computation_time"] > 1.0:
            recommendations.append(
                f"High computation time ({self.computation_stats['average_computation_time']:.3f}s) - consider optimization"
            )
