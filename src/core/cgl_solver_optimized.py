...
def _apply_nonlinear_step_jit(
    phi_real: np.ndarray,
    phi_imag: np.ndarray,
    dt: float,
    c3: float,
    control_real: np.ndarray,
    control_imag: np.ndarray,
) -> None:
    """
...
@njit(parallel=True, fastmath=True, cache=True)
def _apply_linear_step_jit(
    phi_fft_real: np.ndarray,
    phi_fft_imag: np.ndarray,
    exp_op_real: np.ndarray,
    exp_op_imag: np.ndarray,
) -> None:
    """
...
@njit(parallel=True, fastmath=True, cache=True)
def _compute_field_statistics_jit(
    phi_real: np.ndarray, phi_imag: np.ndarray
...
class OptimizedCGLSolver:
    ...
    def _warm_up_jit_functions(self):
        ...
        # Warm up functions
        _apply_nonlinear_step_jit(
            test_real, test_imag, 0.01, 1.0, test_control_real, test_control_imag
        )
    ...
    def step(self, phi: np.ndarray, control_field: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        _apply_linear_step_jit(
            self._phi_fft_real, self._phi_fft_imag, self.exp_half_real, self.exp_half_imag
        )
        ...
        _apply_nonlinear_step_jit(
            self._phi_real, self._phi_imag, self.dt, self.c3, self._control_real, self._control_imag
        )
    ...
    def compute_statistics(self, phi: np.ndarray) -> Dict[str, float]:
        ...
        total_energy, max_intensity, mean_intensity = _compute_field_statistics_jit(
            phi_real, phi_imag
        )
