...
class KuramotoSolver:
    ...
    def __init__(self, config: KuramotoConfig, module_order: List[str]):
        ...
        self.omega = np.array([
            config.natural_frequencies[name] for name in module_order
        ])
    ...
    def set_phase_lags(self, alpha_matrix: np.ndarray):
        ...
        if alpha_matrix.shape != (self.num_modules, self.num_modules):
            raise ValueError(f"Phase lag matrix shape {alpha_matrix.shape} "
    ...
    def build_coupling_matrix(self,
        ...
        if modulation_factors is not None:
            if modulation_factors.shape != (self.num_modules, self.num_modules):
                raise ValueError(f"Modulation factors shape {modulation_factors.shape} "
                               f"doesn't match number of modules {self.num_modules}")
        ...
        if sparsity_mask is not None:
            if sparsity_mask.shape != (self.num_modules, self.num_modules):
                raise ValueError(f"Sparsity mask shape {sparsity_mask.shape} "
                               f"doesn't match number of modules {self.num_modules}")
            K *= sparsity_mask
    ...
    def _compute_derivatives(self, 
                           ...
                           control_signals: np.ndarray) -> np.ndarray:
        """
    ...
    def step(self, 
        ...
        if current_phases.shape != (self.num_modules,):
            raise ValueError(f"Phase vector shape {current_phases.shape} "
                           f"doesn't match number of modules {self.num_modules}")
        
        if coupling_matrix.shape != (self.num_modules, self.num_modules):
            raise ValueError(f"Coupling matrix shape {coupling_matrix.shape} "
                           f"doesn't match number of modules {self.num_modules}")
        
        if control_signals.shape != (self.num_modules,):
            raise ValueError(f"Control signals shape {control_signals.shape} "
                           f"doesn't match number of modules {self.num_modules}")
    ...
    def detect_synchronization_regimes(self, 
        ...
        if phases_history.shape[1] != self.num_modules:
            raise ValueError(f"Phase history has {phases_history.shape[1]} modules, "
