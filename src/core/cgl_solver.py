...
class CGLSolver:
    ...
    def step(self, current_field: np.ndarray, control_field: np.ndarray) -> np.ndarray:
        ...
        if current_field.shape != self.grid_size:
            raise ValueError(f"Field shape {current_field.shape} doesn't match "
                           f"grid size {self.grid_size}")
        
        if control_field.shape != self.grid_size:
            raise ValueError(f"Control field shape {control_field.shape} doesn't match "
                           f"grid size {self.grid_size}")
    ...
    def compute_energy(self, field: np.ndarray) -> float:
        ...
        field_fft = np.fft.fft2(field)
        grad_phi_squared = np.real(
            np.fft.ifft2(self.k_squared * field_fft * np.conj(field_fft))
        )
