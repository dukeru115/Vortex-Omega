...
class SystemState:
    ...
    def __post_init__(self):
        ...
        if self.kuramoto_coupling_matrix.shape != expected_coupling_shape:
            raise ValueError(
                f"Coupling matrix shape {self.kuramoto_coupling_matrix.shape} "
...
def create_empty_system_state(grid_size: Tuple[int, int], n_modules: int) -> SystemState:
    ...
    control_signals = ControlSignals(
        u_field=np.zeros(grid_size, dtype=np.complex128), u_modules=np.zeros(n_modules)
    )
...
def validate_system_state(state: SystemState) -> bool:
    ...
    try:
        ...
        if not np.allclose(
            state.kuramoto_coupling_matrix, state.kuramoto_coupling_matrix.T, rtol=1e-10
        ):
            return False
