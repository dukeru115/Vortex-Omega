...
class GovernanceSystem:
    ...
    def _create_system_admin(self) -> None:
        ...
        if admin_id not in self.stakeholders:
            admin = Stakeholder(
                ...
                permissions={
                    'create_proposal', 'vote_on_proposal', 'manage_stakeholders',
                    'emergency_override', 'system_configuration', 'view_all_data'
                }
            )
    ...
    def _get_default_permissions(self, role: StakeholderRole) -> Set[str]:
        ...
        permission_map = {
            ...
            StakeholderRole.SUPERVISOR: {
                'create_proposal', 'vote_on_proposal', 'view_proposals',
                'emergency_override', 'view_audit_logs'
            },
            ...
            StakeholderRole.AUDITOR: {
                'vote_on_proposal', 'view_proposals', 'view_all_data',
                'view_audit_logs'
            },
            StakeholderRole.USER: {
                'vote_on_proposal', 'view_proposals'
            },
            StakeholderRole.SYSTEM: {
                'view_proposals', 'view_audit_logs'
            }
        }
    ...
    def create_proposal(self,
        ...
        # Set voting deadline
        voting_hours = kwargs.get('voting_period_hours', 
                                 self.governance_config['minimum_voting_period_hours'])
    ...
    def finalize_vote(self, proposal_id: str) -> Dict[str, Any]:
        ...
        # Get eligible voters
        eligible_voters = [s for s in self.stakeholders.values() 
    ...
    def _determine_vote_outcome(self, proposal: GovernanceProposal, yes_votes: int, total_votes: int) -> bool:
        """Determine if proposal passes based on vote type."""
        if total_votes == 0:
            return False
    ...
    def get_governance_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'stakeholder_roles': {role.value: len([s for s in self.stakeholders.values() 
                                                  if s.role == role and s.active]) 
