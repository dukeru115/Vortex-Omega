...
class EarlyWarningPrediction:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommended_horizon": (
                self.recommended_horizon.name if self.recommended_horizon else None
            ),
        }
...
class EarlyWarningSystem:
        ...
        try:
            ...
            if PredictionHorizon.SHORT_TERM in self.current_predictions:
                self.prediction_history.append(
                    self.current_predictions[PredictionHorizon.SHORT_TERM]
                )
    ...
    async def _generate_prediction(
        self, data_array: np.ndarray, horizon: PredictionHorizon
    ) -> Optional[EarlyWarningPrediction]:
        ...
        try:
            ...
            if len(integrity_values) > 2:
                int_slope, int_intercept, int_r_value, _, _ = stats.linregress(
                    time_points, integrity_values
                )
            ...
            if len(coherence_values) > 2:
                coh_slope, coh_intercept, coh_r_value, _, _ = stats.linregress(
    ...
    async def _evaluate_warnings(self):
        ...
        try:
            ...
            if self.constitutional_callback:
                try:
                    thresholds = await self._safe_callback(
                        self.constitutional_callback, "get_thresholds"
                    )
                    if thresholds:
            ...
            for horizon, prediction in self.current_predictions.items():
                await self._evaluate_prediction_warnings(
                    prediction, ha_critical, ha_warning, integrity_critical, integrity_warning
                )

        except Exception as e:
            logger.error(f"Warning evaluation error: {e}")

    async def _evaluate_prediction_warnings(
        self,
        prediction: EarlyWarningPrediction,
        ha_critical: float,
        ha_warning: float,
        integrity_critical: float,
        integrity_warning: float,
    ):
        ...
        if prediction.predicted_ha >= ha_critical * ha_alert_pct:
            ...
            if self._should_create_alert(alert_id):
                ...
                current_ha = (
                    list(self.metrics_buffer)[-1].get("hallucination_number", 0.0)
    ...
    def _estimate_time_to_threshold(
        self, current_value: float, predicted_value: float, threshold: float, horizon_seconds: float
    ) -> Optional[float]:
    ...
    async def _should_resolve_warning(self, warning: EarlyWarningAlert) -> bool:
        ...
        try:
            ...
            if warning.alert_id.startswith("ha_"):
                ...
                if (
                    current_prediction.predicted_integrity
                    > warning.predictions.predicted_integrity * 1.2
                ):
                    return True
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "prediction_confidence": (
                np.mean([p.prediction_confidence for p in self.current_predictions.values()])
...
async def test_early_warning_system():
    ...
    for i in range(30):
        ...
        if i % 5 == 0:  # Print status every 5 iterations
            ...
            print(
                f"  Warnings: {status['active_warnings_count']}, Max Level: {status['max_warning_level']}"
            )
