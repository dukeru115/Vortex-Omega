...
class ControlIntent:
    ...
    u_field_limits: Dict[str, float] = field(
    ...
    kuramoto_masks: Dict[str, Any] = field(
        default_factory=lambda: {
    ...
    # ESC settings
    esc_configuration: Dict[str, Any] = field(
        default_factory=lambda: {
    ...
    freedom_window: Dict[str, float] = field(
        default_factory=lambda: {
            "noise_amplitude": 0.1,
    ...
    emergency_constraints: Dict[str, Any] = field(
        default_factory=lambda: {
    ...
    def get_summary(self) -> str:
        ...
        return f"{self.decision.value}({self.strategy.value}) " f"confidence={self.confidence:.2f}"
...
class ConstitutionV0:
    ...
    def _make_immediate_decision(
        self, risk_payload: RiskMetricsPayload, risk_level: RiskLevel
            ...
            try:
                ...
                if strategy == ControlStrategy.EMERGENCY:
                    intent = self._create_emergency_intent(
                        None, "Comprehensive emergency assessment"
                    )
    ...
    def _analyze_system_state(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "recent_decisions": len(
                [d for d in self.decision_history[-10:] if d.decision == DecisionType.EMERGENCY]
            ),
    ...
    def _determine_strategy(self, analysis: Dict[str, Any]) -> ControlStrategy:
        ...
        if (
            self.state.emergency_mode_start
            and analysis["emergency_duration"] > self.constraints.emergency_mode_max_duration
        ):
        ...
        if (
            analysis["current_risk_level"] == RiskLevel.CRITICAL
            or analysis["recent_decisions"] >= 3  # Many emergency decisions
            or analysis["system_stability"] < 0.3
        ):
            return ControlStrategy.RESTRICTIVE

        # Permissive strategy
        if (
            analysis["current_risk_level"] == RiskLevel.NORMAL
            and analysis["system_stability"] > 0.7
            and analysis["recovery_indicators"] > 0.8
        ):
    ...
    def _calculate_decision_frequency(self) -> float:
        ...
        recent_decisions = [
            d for d in self.decision_history[-20:] if time.time() - d.created_at < 60.0
        ]  # In the last minute
    ...
    def _assess_system_stability(self) -> float:
        ...
        emergency_count = sum(1 for d in recent_decisions if d.decision == DecisionType.EMERGENCY)
    ...
    def _create_emergency_intent(
        self, risk_payload: Optional[RiskMetricsPayload], reason: str
    ) -> ControlIntent:
        """Create emergency intention"""

        intent = ControlIntent(
            decision=DecisionType.EMERGENCY, strategy=ControlStrategy.EMERGENCY, confidence=0.95
        )
    ...
    def _create_restrictive_intent(
        self, risk_payload: Optional[RiskMetricsPayload], reason: str
    ) -> ControlIntent:
        """Create restrictive intention"""

        intent = ControlIntent(
            decision=DecisionType.REJECT, strategy=ControlStrategy.RESTRICTIVE, confidence=0.8
        )
    ...
    def _create_standard_intent(self, reason: str) -> ControlIntent:
        ...
        intent = ControlIntent(
            decision=DecisionType.ACCEPT, strategy=ControlStrategy.STANDARD, confidence=0.9
        )
        ...
        if self.state.emergency_mode_start is not None:
            self.logger.info(
                f"Exiting emergency mode after {self.state.get_emergency_duration():.1f}s"
            )
    ...
    def _create_permissive_intent(self, reason: str) -> ControlIntent:
        ...
        intent = ControlIntent(
            decision=DecisionType.ACCEPT, strategy=ControlStrategy.PERMISSIVE, confidence=0.85
        )
    ...
    def manual_decision(
        self, risk_level: RiskLevel, additional_context: Optional[Dict[str, Any]] = None
    ) -> ControlIntent:
        """
        ...
        with self._lock:
            status = {
                ...
                "statistics": {
                    ...
                    "accept_rate": (
                        self.state.accept_decisions / max(1, self.state.total_decisions)
                    ),
                    "reject_rate": (
                        self.state.reject_decisions / max(1, self.state.total_decisions)
                    ),
                    "emergency_rate": (
                        self.state.emergency_decisions / max(1, self.state.total_decisions)
                    ),
                    "decision_frequency": self._calculate_decision_frequency(),
                    "system_stability": self._assess_system_stability(),
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"ConstitutionV0(risk={self.state.current_risk_level.value}, "
