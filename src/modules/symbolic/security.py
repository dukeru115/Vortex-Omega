...
class RateLimiter:
    ...
    def check_rate_limit(self, user_id: str, current_time: float) -> bool:
        ...
        self.requests[user_id] = [(t, c) for t, c in self.requests[user_id] if t > window_start]
    ...
    def get_remaining(self, user_id: str, current_time: float) -> int:
        ...
        window_start = current_time - self.window_seconds
        valid_requests = [(t, c) for t, c in self.requests[user_id] if t > window_start]
...
class SecurityValidator:
    ...
    def validate_input(self, text: str, context: SecurityContext) -> Dict[str, Any]:
        """
        ...
        Returns:
            Validation result
        """
        result = {"valid": False, "sanitized_text": None, "errors": [], "warnings": []}

        try:
            # Check rate limiting
            if context.user_id:
                if not self.rate_limiter.check_rate_limit(context.user_id, time.time()):
