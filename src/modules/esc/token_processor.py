...
class TokenProcessor:
    ...
    def process_tokens(self, 
                      tokens: List[str], 
                      context: Optional[Dict[str, Any]] = None) -> List[TokenInfo]:
        """
        ...
        for i, token in enumerate(tokens):
            ...
            if self.enable_safety_checking:
                if constitutional_score < self.constitutional_threshold:
                    ...
                    logger.warning(f"Constitutional violation filtered: '{token}' -> '{normalized_token}'")
    ...
    def classify_token_type(self, 
                           token: str, 
                           position: int, 
                           full_sequence: List[str]) -> TokenType:
        """
    ...
    def compute_constitutional_score(self, 
                                   token: str, 
        ...
        positive_matches = sum(1 for keyword in self.safety_keywords['positive'] 
                             if keyword in token_lower)
        negative_matches = sum(1 for keyword in self.safety_keywords['negative'] 
                             if keyword in token_lower)
        ...
        if context:
            ...
            if context_type in self.context_safety_rules:
                ...
                allowed_matches = sum(1 for keyword in rules['allowed'] 
                                    if keyword in token_lower)
                forbidden_matches = sum(1 for keyword in rules['forbidden'] 
                                      if keyword in token_lower)
    ...
    def compute_echo_strength(self, 
                            token: str, 
                            position: int, 
                            full_sequence: List[str]) -> float:
        """
    ...
    def detect_echo_pattern(self, 
                          token: str, 
                          position: int, 
                          full_sequence: List[str]) -> bool:
        """
    ...
    def is_proper_noun(self, token: str) -> bool:
        ...
        return (len(token) > 1 and 
                token[0].isupper() and 
                any(c.islower() for c in token[1:]) and
                not token.isupper())  # Not an acronym
