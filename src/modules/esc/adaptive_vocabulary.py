...
class AdaptiveVocabulary:
    ...
    def evaluate_token(
        self, token: str, context: Optional[str] = None, force_evaluation: bool = False
    ) -> Tuple[TokenEntry, Dict[str, Any]]:
        ...
        if token_entry.status == TokenStatus.REJECTED:
            logger.warning(
                f"Token rejected: '{token}' (constitutional: {token_entry.constitutional_score:.3f}, risk: {token_entry.risk_score:.3f})"
            )
    ...
    def _evaluate_constitutional_compliance(
        self, token: str, context: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def _evaluate_safety_risks(self, token: str, context: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _determine_token_status(
        ...
        # Normal evaluation modes
            ...
            # Check constitutional compliance
                ...
                if self.current_mode == VocabularyMode.CONSERVATIVE:
                    ...
                    decision_factors.append(
                        f"Conservative mode - constitutional score too low: {token_entry.constitutional_score:.3f}"
                    )
                else:
                    status = TokenStatus.QUARANTINE
                    decision_factors.append(
                        f"Quarantined for constitutional review: {token_entry.constitutional_score:.3f}"
                    )
            ...
            # Constitutional violations
                ...
                status = TokenStatus.QUARANTINE
                decision_factors.append(
                    f"Constitutional negative patterns: {constitutional_analysis['negative_matches']}"
                )
            ...
            # Approved cases
                ...
                if self.current_mode == VocabularyMode.CONSERVATIVE:
                    ...
                    if token_entry.constitutional_score > 0.3:
                        status = TokenStatus.APPROVED
                        decision_factors.append(
                            "Exploratory approval - moderate constitutional score"
                        )
                    else:
                        status = TokenStatus.QUARANTINE
                    ...
                    if (
                        token_entry.constitutional_score
                        >= self.config.constitutional_score_threshold
                    ):
                        status = TokenStatus.APPROVED
                        decision_factors.append(
                            "Balanced approval - meets constitutional threshold"
                        )
                    else:
                        status = TokenStatus.PENDING
    ...
    def _handle_vocabulary_overflow(self, new_token_entry: TokenEntry):
        ...
        if self.expanded_vocabulary:
            ...
            if token_scores and new_score > token_scores[0][0]:
                ...
                logger.info(
                    f"Vocabulary overflow: replaced '{old_token}' (score {token_scores[0][0]:.3f}) with '{new_token_entry.token}' (score {new_score:.3f})"
                )
            else:
    ...
    def _generate_token_id(self, token: str) -> int:
        ...
        while any(
            ...
            token_id = (token_id + 1) % (
                self.config.base_vocabulary_size + self.config.max_expansion_size
            )
    ...
    def reset_vocabulary_expansion(self, preserve_approved: bool = True):
        ...
        if preserve_approved:
            ...
            approved_tokens = {
                k: v
                for k, v in self.expanded_vocabulary.items()
    ...
    def get_vocabulary_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_state": {
                ...
                "expansion_utilization": len(self.expanded_vocabulary)
    ...
    def generate_vocabulary_report(self) -> Dict[str, Any]:
        ...
        approved_tokens = [
            entry
            for entry in self.expanded_vocabulary.values()
            if entry.status == TokenStatus.APPROVED
        ]
        ...
        for token, entry in self.rejected_tokens.items():
            if entry.constitutional_score < self.config.constitutional_score_threshold:
                rejection_analysis["low_constitutional"] = (
                    rejection_analysis.get("low_constitutional", 0) + 1
                )
        ...
        return {
            ...
            "mode": self.current_mode.value,
            "vocabulary_metrics": {
                "total_tokens_managed": len(self.base_vocabulary)
                + len(self.expanded_vocabulary)
                + len(self.quarantine_tokens)
                + len(self.rejected_tokens),
                "expansion_efficiency": len(self.expanded_vocabulary)
                / max(1, self.stats["tokens_evaluated"]),
                "approval_rate": self.stats["tokens_approved"]
                / max(1, self.stats["tokens_evaluated"]),
                "rejection_rate": self.stats["tokens_rejected"]
                / max(1, self.stats["tokens_evaluated"]),
            },
            "quality_analysis": {
    ...
    def _generate_vocabulary_recommendations(self) -> List[str]:
        ...
        if approval_rate < 0.1:
            recommendations.append(
                f"Very low approval rate ({approval_rate:.3f}) - consider more permissive mode"
            )
        elif approval_rate > 0.8:
            recommendations.append(
                f"Very high approval rate ({approval_rate:.3f}) - consider stricter evaluation"
            )

        # Check quarantine backlog
        if len(self.quarantine_tokens) > 100:
            recommendations.append(
                f"Large quarantine backlog ({len(self.quarantine_tokens)}) - review quarantine policies"
            )
        ...
        if utilization > 0.9:
            recommendations.append(
                f"Vocabulary expansion near capacity ({utilization:.3f}) - consider increasing limit or pruning"
            )

        # Check emergency lockdowns
        if self.stats["emergency_lockdowns"] > 3:
            recommendations.append(
                "Multiple emergency lockdowns detected - review safety thresholds"
            )
        ...
        if constitutional_violations > len(self.expanded_vocabulary) * 0.1:
            recommendations.append(
                f"High constitutional violation rate in vocabulary - review approval process"
            )
