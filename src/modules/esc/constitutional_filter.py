...
class ConstitutionalContentFilter:
    ...
    def _analyze_content_risk(
        ...
        dangerous_risk = max(
            [
                ...
                if key.startswith("dangerous_") and isinstance(score, float)
            ],
            default=0.0,
        )
        moderate_risk = max(
            [
                score
                for key, score in risk_scores.items()
                if key.startswith("moderate_") and isinstance(score, float)
            ],
            default=0.0,
        )
    ...
    def _apply_contextual_modifiers(
        self, risk_scores: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
                ...
                if isinstance(risk_scores[risk_key], float):
                    ...
                    for mod_key, mod_value in modifiers.items():
                        if mod_key.replace("_modifier", "") in risk_key:
                            risk_scores[risk_key] = max(
                                0.0, min(1.0, risk_scores[risk_key] + mod_value)
                            )

        return risk_scores

    def _apply_filtering_policy(
        self, content: str, risk_analysis: Dict[str, Any], context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        ...
        if overall_risk >= policy_config["dangerous_threshold"]:
            ...
            filter_reason.append(
                f"Risk level {overall_risk:.3f} exceeds dangerous threshold {policy_config['dangerous_threshold']}"
            )

        if risk_analysis["dangerous_risk"] >= 0.5:  # Always filter high dangerous content
            should_filter = True
            filter_reason.append(
                f"Dangerous content detected: {risk_analysis['dangerous_risk']:.3f}"
            )

        # Apply filtering if needed
        if should_filter:
            filtered_content, replacements = self._filter_dangerous_content(
                content, risk_analysis, policy_config
            )
    ...
    def _filter_dangerous_content(
        self, content: str, risk_analysis: Dict[str, Any], policy_config: Dict[str, Any]
        ...
        for category, patterns in self.dangerous_patterns.items():
            for pattern in patterns:
                ...
                for match in reversed(matches):  # Reverse to maintain indices
                    ...
                    filtered_content = (
                        filtered_content[:start] + replacement_text + filtered_content[end:]
                    )
    ...
    def _check_constitutional_violations(self, risk_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if risk_analysis["dangerous_risk"] > 0.8:
            violations.append(
                f"Dangerous content risk {risk_analysis['dangerous_risk']:.3f} exceeds constitutional limit"
            )

        # Check overall risk
        if risk_analysis["overall_risk"] > self.config.risk_threshold:
            violations.append(
                f"Overall risk {risk_analysis['overall_risk']:.3f} exceeds threshold {self.config.risk_threshold}"
            )

        # Check constitutional positive balance
        if risk_analysis["overall_risk"] > 0.5 and risk_analysis["constitutional_positive"] < 0.1:
    ...
    def _check_emergency_conditions(self, risk_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if risk_analysis["overall_risk"] > self.config.emergency_threshold:
            emergency_conditions.append(
                f"Overall risk {risk_analysis['overall_risk']:.3f} exceeds emergency threshold"
            )

        # Multiple dangerous categories detected
        dangerous_categories = [
            key
            for key, value in risk_analysis["risk_scores"].items()
            if key.startswith("dangerous_") and isinstance(value, float) and value > 0.5
        ]

        if len(dangerous_categories) >= 3:
            emergency_conditions.append(
                f"Multiple dangerous categories detected: {dangerous_categories}"
            )
    ...
    def _update_adaptive_learning(
        self, content: str, risk_analysis: Dict[str, Any], filtering_result: Dict[str, Any]
    ):
        ...
        if len(self.policy_effectiveness[self.current_policy.value]) > 100:
            self.policy_effectiveness[self.current_policy.value] = self.policy_effectiveness[
    ...
    def get_filter_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "violation_summary": {
                ...
                "recent_violations": (
                    list(self.violation_history)[-10:] if self.violation_history else []
                ),
                "violation_rate": len(self.violation_history)
                / max(1, self.stats["total_content_processed"]),
            },
            "emergency_summary": {
                "total_interventions": len(self.intervention_history),
                "recent_interventions": (
                    list(self.intervention_history)[-5:] if self.intervention_history else []
                ),
            },
            "filtering_efficiency": {
                "content_filtered_rate": self.stats["content_filtered"]
                / max(1, self.stats["total_content_processed"]),
                "constitutional_violation_rate": self.stats["constitutional_violations"]
                / max(1, self.stats["total_content_processed"]),
            },
        }
    ...
    def generate_filter_report(self) -> Dict[str, Any]:
        ...
        filtered_rate = sum(1 for entry in recent_history if entry["filtered"]) / len(
            recent_history
        )
        ...
        for policy, effectiveness_list in self.policy_effectiveness.items():
            if effectiveness_list:
                policy_performance[policy] = {
                    ...
                    "filtering_rate": sum(1 for e in effectiveness_list if e["filtered"])
                    / len(effectiveness_list),
                    "constitutional_compliance": sum(
                        1 for e in effectiveness_list if e["constitutional_compliant"]
                    )
                    / len(effectiveness_list),
                }
        ...
        return {
            ...
            "recent_performance": {
                ...
                "risk_distribution": {
                    level.value: risk_levels.count(level) for level in ContentRiskLevel
                },
    ...
    def _generate_filter_recommendations(self) -> List[str]:
        ...
        if violation_rate > self.config.max_violation_rate:
            recommendations.append(
                f"High violation rate ({violation_rate:.3f}) - consider stricter policy"
            )
        ...
        if self.current_policy == FilterPolicy.ADAPTIVE:
            if len(self.policy_effectiveness.get("adaptive", [])) > 20:
                ...
                compliance_rate = sum(
                    1 for e in effectiveness if e["constitutional_compliant"]
                ) / len(effectiveness)
                if compliance_rate < 0.8:
                    recommendations.append(
                        f"Adaptive policy showing low compliance ({compliance_rate:.3f}) - consider manual adjustment"
                    )

        # Check filtering balance
        filtering_rate = self.stats["content_filtered"] / max(
            1, self.stats["total_content_processed"]
        )
        if filtering_rate > 0.5:
            recommendations.append(
                f"High filtering rate ({filtering_rate:.3f}) - may be too restrictive"
            )
        elif filtering_rate < 0.05:
            recommendations.append(
                f"Very low filtering rate ({filtering_rate:.3f}) - may be too permissive"
            )
