...
class ConstitutionalRealTimeMonitor:
    ...
    async def _monitoring_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                sleep_time = max(
                    0, self.config.monitoring_interval_ms / 1000.0 - (time.time() - start_time)
                )
    ...
    async def _collect_system_metrics(self):
        ...
        try:
            ...
            if self.metrics_callback:
                ...
                if metrics_data:
                    # Update core metrics
                    self.current_metrics.hallucination_number = metrics_data.get(
                        "hallucination_number", 0.0
                    )
                    self.current_metrics.coherence_measure = metrics_data.get(
    ...
    async def _apply_constitutional_algorithm(self) -> Dict[str, Any]:
        ...
        if ha_value > self.config.ha_emergency_threshold:
            ...
            decision["reasoning"].append(
                f"Ha={ha_value:.3f} > emergency_threshold={self.config.ha_emergency_threshold}"
            )
        ...
        # 2. Check integrity threshold (Forced synchronization)
            ...
            for module in target_modules:
                decision["control_signals"][module] = sync_signal * np.cos(
                    np.linspace(0, 2 * np.pi, 10)
                )

            decision["type"] = "FORCED_SYNC"
            decision["reasoning"].append(
                f"integrity={integrity_score:.3f} < threshold={self.config.integrity_critical_threshold}"
            )

        # 3. Warning states (Enhanced monitoring)
        elif (
            ha_value > self.config.ha_warning_threshold
            or integrity_score < self.config.integrity_warning_threshold
        ):

            decision["type"] = "MONITOR"
            decision["control_signals"] = {}  # No control intervention
            decision["reasoning"].append(
                f"Warning state: Ha={ha_value:.3f}, integrity={integrity_score:.3f}"
            )
    ...
    async def _execute_constitutional_decision(self, decision: Dict[str, Any]):
        ...
        try:
            ...
            # Handle recovery from emergency
                ...
                if (
                    emergency_duration > self.config.emergency_timeout_seconds
                    or self.current_metrics.hallucination_number < self.config.ha_warning_threshold
                ):
    ...
    def _update_system_status(self):
        ...
        if self.emergency_active:
            ...
            ha >= self.config.ha_failure_threshold
            or integrity <= self.config.integrity_failure_threshold
        ):
            self.current_status = ConstitutionalStatus.FAILURE
        elif (
            ha >= self.config.ha_critical_threshold
            or integrity <= self.config.integrity_critical_threshold
        ):
            self.current_status = ConstitutionalStatus.CRITICAL
        elif (
            ha >= self.config.ha_warning_threshold
            or integrity <= self.config.integrity_warning_threshold
        ):
    ...
    async def _check_integrity_alerts(self):
        ...
        if (
            integrity <= self.config.integrity_critical_threshold
            and "integrity_critical" not in self.active_alerts
        ):
            alert = ConstitutionalAlert(
    ...
    async def _check_performance_alerts(self):
        ...
        if (
            latency > self.config.max_processing_latency_ms
            and "performance_latency" not in self.active_alerts
        ):
            alert = ConstitutionalAlert(
    ...
    async def _should_resolve_alert(self, alert: ConstitutionalAlert) -> bool:
        ...
        if alert.alert_id == "ha_critical" and ha < self.config.ha_warning_threshold:
            ...
            alert.alert_id == "integrity_critical"
            and integrity > self.config.integrity_warning_threshold
        ):
            return True
        elif (
            alert.alert_id == "performance_latency"
            and latency < self.config.max_processing_latency_ms * 0.8
        ):
            return True
    ...
    def get_metrics_history(self, hours: int = 1, include_alerts: bool = True) -> Dict[str, Any]:
        """Get historical metrics and alerts"""
        cutoff_time = time.time() - (hours * 3600)

        # Filter metrics by time
        filtered_metrics = [m.to_dict() for m in self.metrics_history if m.timestamp >= cutoff_time]
...
async def test_constitutional_monitoring():
    ...
    for i in range(10):
        ...
        print(
            f"Status: {status['status']}, Ha: {status['metrics']['hallucination_number']:.3f}, "
