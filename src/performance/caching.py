    ...
    def _compute_size(self, value: Any) -> int:
        ...
        try:
            if isinstance(value, np.ndarray):
                ...
                return sum(self._compute_size(k) + self._compute_size(v) 
...
class EmbeddingCache:
    ...
    def get_embedding(self, tokens: List[str], context: Optional[str] = None) -> Optional[np.ndarray]:
    ...
    def put_embedding(self, tokens: List[str], embedding: np.ndarray, 
                     context: Optional[str] = None, ttl: Optional[float] = 3600):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_hit_rate': (l1_stats['hits'] + l2_stats['hits']) / 
                             max(1, l1_stats['hits'] + l1_stats['misses'] + 
...
class ComputationCache:
    ...
    def put_result(self, func_name: str, args: Tuple, kwargs: Dict, result: Any,
                  dependencies: Optional[List[str]] = None, ttl: Optional[float] = None):
    ...
    def cached_call(self, func: Callable, *args, 
                   dependencies: Optional[List[str]] = None,
                   ttl: Optional[float] = None, **kwargs):
...
class CacheManager:
    ...
    async def _memory_monitor_loop(self):
        ...
        while self._running:
            try:
                ...
                self.logger.debug(f"Cache stats - Total: {stats['total_size_mb']:.1f}MB, "
    ...
    def get_comprehensive_stats(self) -> Dict[str, Any]:
        ...
        total_size_mb = (embedding_stats['l1_cache']['size_mb'] + 
                        embedding_stats['l2_cache']['size_mb'] + 
                        computation_stats['size_mb'])
        
        # Overall hit rate calculation
        total_hits = (embedding_stats['l1_cache']['hits'] + 
                     embedding_stats['l2_cache']['hits'] + 
                     computation_stats['hits'])
        total_requests = (embedding_stats['l1_cache']['hits'] + embedding_stats['l1_cache']['misses'] +
                         embedding_stats['l2_cache']['hits'] + embedding_stats['l2_cache']['misses'] +
                         computation_stats['hits'] + computation_stats['misses'])
    ...
    def optimize_performance(self):
        ...
        self.logger.info(f"Performance optimization - Embedding HR: {emb_hit_rate:.3f}, "
