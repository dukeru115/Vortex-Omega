...
class BenchmarkSuite:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fastest_benchmark": (
                min(self.results, key=lambda x: x.execution_time).name if self.results else None
            ),
            "slowest_benchmark": (
                max(self.results, key=lambda x: x.execution_time).name if self.results else None
            ),
        }
...
class PerformanceBenchmark:
    ...
    def benchmark_cgl_solver(self, grid_sizes: List[Tuple[int, int]] = None) -> BenchmarkSuite:
        ...
        for nx, ny in grid_sizes:
            ...
            # Setup configuration
            config = CGLConfig(
                grid_size=(nx, ny), spatial_extent=(10.0, 10.0), time_step=0.01, c1=1.0, c3=1.0
            )
            ...
            result = self.benchmark_function(single_step, name=f"CGL_step_{nx}x{ny}")
            ...
            result = self.benchmark_function(evolution_10_steps, name=f"CGL_evolve_10_{nx}x{ny}")
            suite.add_result(result)
    ...
    def benchmark_kuramoto_solver(self, oscillator_counts: List[int] = None) -> BenchmarkSuite:
        ...
        for n_osc in oscillator_counts:
            ...
            # Setup configuration
            config = KuramotoConfig(n_oscillators=n_osc, coupling_strength=1.0, time_step=0.01)
            ...
            result = self.benchmark_function(single_step, name=f"Kuramoto_step_{n_osc}")
            suite.add_result(result)
            ...
            result = self.benchmark_function(
                evolution_100_steps, name=f"Kuramoto_evolve_100_{n_osc}"
            )
            suite.add_result(result)
    ...
    def benchmark_memory_scaling(self) -> BenchmarkSuite:
        ...
        for size in data_sizes:
            ...
            result = self.benchmark_function(allocate_array, name=f"numpy_alloc_{size}")
            ...
            result = self.benchmark_function(complex_operations, name=f"fft_complex_{size}")
            result.additional_metrics["array_size"] = size
            suite.add_result(result)
    ...
    def compare_optimized_vs_standard(self) -> BenchmarkSuite:
        ...
        result = self.benchmark_function(
            lambda: solver.evolve(phi, 50), name="optimized_cgl_evolution"
        )
        suite.add_result(result)
        ...
        result = self.benchmark_function(
            lambda: solver.evolve(phases, 200), name="optimized_kuramoto_evolution"
        )
        suite.add_result(result)
    ...
    def generate_report(
        self, benchmarks: Dict[str, BenchmarkSuite], save_path: Optional[str] = None
    ) -> str:
        ...
        report_lines.append(
            f"System: {psutil.cpu_count()} cores, {psutil.virtual_memory().total // (1024**3)}GB RAM"
        )
        report_lines.append("")
        ...
        for suite_name, suite in benchmarks.items():
            ...
            report_lines.append(
                f"- Average memory usage: {summary['average_memory_usage_mb']:.1f}MB"
            )
    ...
    def plot_performance_comparison(
        ...
        try:
            ...
            if "cgl" in benchmarks:
                cgl_results = [
                    r for r in benchmarks["cgl"].results if "step_" in r.name and r.error is None
                ]
                if cgl_results:
            ...
            if "kuramoto" in benchmarks:
                kuramoto_results = [
                    r
                    for r in benchmarks["kuramoto"].results
                    if "step_" in r.name and r.error is None
                ]
                if kuramoto_results:
                    oscillators = [
                        r.additional_metrics.get("oscillators", 0) for r in kuramoto_results
                    ]
...
def quick_benchmark() -> Dict[str, Any]:
    ...
    cgl_result = benchmark.benchmark_function(lambda: solver.step(phi), name="quick_cgl_test")
    ...
    kuramoto_result = benchmark.benchmark_function(
        lambda: solver.step(phases), name="quick_kuramoto_test"
    )
