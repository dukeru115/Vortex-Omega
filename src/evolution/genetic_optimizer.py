...
class GeneticOptimizer:
    ...
    def initialize_population(self) -> None:
        ...
        if self.strategy == EvolutionaryStrategy.PARTICLE_SWARM:
            self.pso_params['velocities'] = [
                np.random.uniform(-1, 1, self.genome_length)
                for _ in range(self.population_size)
            ]
        elif self.strategy == EvolutionaryStrategy.CMA_ES:
            self.cma_params['mean'] = np.mean([ind.genome for ind in self.population.individuals], axis=0)
    ...
    def evolve(self) -> Dict[str, Any]:
        ...
        logger.info(f"Evolution completed in {execution_time:.2f}s after {self.generation + 1} generations")
    ...
    def _evolve_cma_es(self) -> None:
        ...
        best_genomes = [ind.genome for ind in self.population.individuals[:self.population_size//2]]
    ...
    def _evolve_hybrid_strategy(self) -> None:
        ...
        if self.pso_params['velocities'] is None or len(self.pso_params['velocities']) != len(pso_individuals):
            self.pso_params['velocities'] = [
    ...
    def _tournament_selection(self, tournament_size: int = 3) -> List[Individual]:
        ...
        selected = []
        for _ in range(self.population_size):
            tournament = random.sample(self.population.individuals, min(tournament_size, len(self.population.individuals)))
    ...
    def _blend_alpha_crossover(self, parent1: Individual, parent2: Individual, alpha: float = 0.5) -> Tuple[Individual, Individual]:
    ...
    def _adapt_operators(self) -> None:
        ...
        logger.debug(f"Adapted parameters - CR: {self.crossover_rate:.3f}, MR: {self.mutation_rate:.3f}")
